...
Type
  Numero = class
    ...
    public
      ...
      Function Invertido: Cardinal;
      Function ImparOnPosPar: Cardinal;
...
Begin
  ...
  While Value > 0 do
  Begin
    aux:= aux * 10 + Value mod 10;
    Value:= Value div 10
  End;
  Value:= aux;
End;

Procedure Numero.Rotar(veces: Byte);
Var
  i, k: Cardinal;
Begin
  k:= Potencia(10, NumDig-1); i:= 1;
  while (veces < NumDig) and (i <= veces) do
  begin
    Value:= (Value mod 10) * k + Value div 10;
    i:= i + 1;
  end;
End;
...
Function Numero.Invertido: Cardinal;
Var
  aux, inv: Cardinal;
Begin
  aux:= Value;
...
Function Numero.ImparOnPosPar: Cardinal;
Var
  cont: Cardinal;
  i: byte;
Begin
  ...
  while i <= NumDig do
  begin
    if (Digito(i) mod 2 = 1) and (i mod 2 = 0) then
      cont:= cont + 1;
    i:= i + 1;
  end;
  Invertir;
